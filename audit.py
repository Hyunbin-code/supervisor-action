...
def main():
    ...
    logger.info(f"🔍 Starting Audit for {repo_name} PR #{pr_number}")
    ...
    logger.info(f"✅ Audit Complete. Risk Score: {risk_score}")

    # 3. Comment on GitHub (Only if issues found or High Risk)
    if risk_score > 0 and len(result.get("issues", [])) > 0:
        if not dry_run:
            body = f"## 🛡️ Supervisor Audit Report (Risk: {risk_score}/100)\n\n"
